...
public partial class Forecast : ComponentBase, IPageComponent, IAsyncDisposable
{
   ...
   private bool _isGeneratingForecast; 
   ...
   private string _buttonGenerateForecastString = "";
   ...
   private async Task LoadLocalizedStringsAsync()
   {
       ...
       _buttonGenerateForecastString = await Localizer.GetStringAsync("forecast.form.button_generate_forecast"); 
   ...
   private async Task LoadCurrencyListAsync()
   {
       ...
       string startDateString = DateTime.Today.AddMonths(-1).ToString(DateConstants.DateFormat);
       string endDateString = DateTime.Today.ToString(DateConstants.DateFormat);
       
       string url =
           $"{Configuration.ApiUrl}/Rate/get-all-currencies-in-range?StartDateString={startDateString}&EndDateString={endDateString}";
       try
       {
   ...
   private async Task TriggerForecastAsync()
   {
